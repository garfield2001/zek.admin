            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <Input placeholder="Search dishes..." v-model="searchQuery" class="max-w-sm border-primary" />
                    </div>
                    <div class="flex items-center gap-2">
                                ...
                                <TableRow class="bg-muted/50">
                                    <TableHead v-for="column in table.getAllColumns()" :key="column.id" :class="{
                                        ...
                                        <template v-if="column.id === 'select'">
                                            <Checkbox :checked="selectedDishes.size === table.getRowModel().rows.length"
                            ...
                            <TableBody>
                                <TableRow v-for="row in table.getRowModel().rows" :key="row.id"
                                    ...
                                    <TableCell v-for="column in table.getAllColumns()" :key="column.id" :class="{
                                        ...
                                        <template v-if="column.id === 'select'">
                                            <Checkbox :checked="selectedDishes.has(row.original.name)" @update:checked="(checked: boolean) => {
                                                if (checked) {
        ...
        <Dialog v-model:open="showEditDialog">
            <DialogContent class="max-h-[90vh] overflow-y-auto">
                ...
                <DialogFooter>
                    ...
                    <Button @click="saveDish" :disabled="!editingDish?.name?.trim() || !editingDish?.category?.trim()">
                        Save Changes
                    </Button>
                </DialogFooter>
            </DialogContent>
